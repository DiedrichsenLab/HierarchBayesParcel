...
def do_simulation_sessFusion_sess(K=5, M=np.array([5],dtype=int), nsubj_list=None, num_part=3,
    ...
    T = make_full_model(K=K, P=grid.P, nsubj_list=nsubj_list, num_part=num_part, M=M)
    T.arrange.logpi = pm.logpi
...
def simulation_2(width=30,
                 nsub_list=np.array([10]),
                 M=np.array([10],dtype=int),
                 num_part=3):
    """Simulation of common kappa vs. separate kappas across subjects
    Args:
        width: The width and height of MRF grid
    ...
    Returns:
        Simulation result plots
    """
    grid, U, U_indv, Uerrors, Props = do_simulation_sessFusion_sess(K=5, M=M,
                                                                    nsubj_list=nsub_list,
                                                                    num_part=num_part,
                                                                    width=width,
                                                                    low_kappa=3,
